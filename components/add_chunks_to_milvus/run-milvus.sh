...
EOF
    ...
    cat << EOF > ./.milvus/user.yaml
# Extra config to override default milvus.yaml
EOF

    mkdir -p $(pwd)/.milvus/volumes/milvus

    if [ ! -f "./.milvus/embedEtcd.yaml" ]
    then
        echo "embedEtcd.yaml file does not exist. Please try to create it in the current directory."
        exit 1
    fi

    if [ ! -f "./.milvus/user.yaml" ]
    then
    ...
    podman run -d \
        ...
        -v $(pwd)/.milvus/user.yaml:/milvus/configs/user.yaml \
        -p 19530:19530 \
        -p 9091:9091 \
        -p 2379:2379 \
